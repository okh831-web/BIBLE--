...
2. **절대 제거 대상 (제목에 포함 금지):** 
   - 번호 형식 (예: '- 1 -', '1.', '첫째')
   - 날짜 및 예배 명칭 (예: '2025년 3월 9일', '주일말씀')
   - 괄호/꺾쇠 안의 성구 (예: '<잠언 4장 23절>', '(에베소서 1장 18절)')
3. **최종 제목 예시:** 
   - 입력: "- 1 - <2025년 3월 9일 주일말씀> 생각해야 그것에 대해 눈이 떠진다 <잠언 4장 23절>"
   - 출력: "생각해야 그것에 대해 눈이 떠진다"
4. **내용 출처:** 오직 제공된 텍스트 내에서만 내용을 추출하고 외부 지식을 절대 섞지 마십시오.
        ...
        properties: {
          summaryCard: {
            ...
            properties: {
              ...
              subject: { type: Type.STRING, description: "정제된 순수 말씀 주제 (번호, 성구 제거)" },
              coreMessage: { type: Type.ARRAY, items: { type: Type.STRING }, description: "핵심 요약 3문장" },
              scripture: { type: Type.STRING, description: "본문에 언급된 핵심 성구 구절" },
          ...
          infographic: {
            type: Type.OBJECT,
            properties: {
              title: { type: Type.STRING, description: "정제된 순수 말씀 주제 (번호, 성구 제거)" },
              subtitle: { type: Type.STRING, description: "본문 기반 감동적인 요약 문구" },
